...
The study focuses on the design and development of a Web-Based Accessibility Evaluation Tool that analyzes website content using selected parameters derived from WCAG 2.1 guidelines. The WCAG standard defines three levels of conformance: Level A (the minimum set of requirements for basic accessibility), Level AA (the standard recommended for all web content, which addresses the biggest and most common barriers for people with disabilities), and Level AAA (the highest level, which requires specialized support and is not typically achievable for all content). This project specifically targets the criteria within Levels A and AA. The system will be developed using web technologies such as HTML, CSS, and JavaScript for the front-end and a suitable backend framework such as PHP with MySQL for processing and data management.

The system evaluates web pages through two primary input parameters: website URL input and direct HTML code input. The processing involves predefined accessibility checks such as verifying alternative text for images, evaluating heading structure, identifying unlabeled form elements, verifying descriptive link text, detecting missing page title or language attributes, and performing basic color contrast checks. The system outputs evaluation summaries, categorized issues, and WCAG references using clear, sentence-form language.

The scope of this study is informed by empirical research on accessibility education, which has systematically identified specific learning barriers and preferences that shaped the system's educational features. The tool is designed to address the needs of students who lack practical examples (the most prevalent barrier at 42.2%), those who need hands-on checking tools (26.7%), and those who find guidelines difficult to understand without proper pedagogical support (24.4%). Additionally, the system responds to the 40% of students who identified limited classroom discussion as a barrier by providing supplementary educational resources that can extend learning beyond traditional classroom instruction. The emphasis on step-by-step explanations, simple language, and immediate feedback reflects the pedagogical preferences expressed by the majority of student respondents, ensuring that the tool's design is grounded in evidence-based educational principles rather than assumptions about learner needs.

However, the system is not intended in several ways: it does not automatically repair accessibility issues but only identifies and explains them; technical limitations of PHP-based parsing mean it cannot simulate full assistive technology behavior or deeply analyze complex CSS and ARIA interactions; performance may be affected by large HTML structures or internet connectivity during URL-based evaluations; and its usability relies on the accuracy of the user-provided URL or HTML code. The tool is specifically designed for beginners, students, and small-scale developers, and does not aim to produce full enterprise-grade WCAG audit reports. Its ultimate purpose is to deliver a lightweight, educational, and easy-to-use tool that evaluates accessibility through code inspection, structural interpretation, and clear recommendations, recognizing the constraints of technology, time, and available resources.
