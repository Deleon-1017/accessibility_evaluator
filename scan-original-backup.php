...
try {
    ...
    // Load accessibility checks from database via API
    $allIssues = [];
    ...
    $apiUrl = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/api/get-accessibility-checks.php';
    $checksJson = @file_get_contents($apiUrl);
    
    if ($checksJson === false) {
        error_log("Failed to fetch accessibility checks from API: $apiUrl");
        throw new Exception('Unable to load accessibility checks. Please ensure the database is configured.');
    }
